  ...
  function (PluginManager, Delay, WordGetter) {
    PluginManager.add('wordcount', function (editor) {
      ...
      var getCount = function () {
        var pom = (editor.getContent()).replace(/(<([^>]+)>)/ig, "");
        pom = pom.replace(/\n/g,"");
        pom = pom.replace('&nbsp;', '');

        var liczbaZnakow = pom.length;
        return liczbaZnakow;
      };
      ...
      var update = function () {
        editor.theme.panel.find('#wordcount').text(['Liczba znaków: {0}, Liczba słów: {1}', getCount(), getWord()]);
      };
        ...
        if (statusbar) {
          Delay.setEditorTimeout(editor, function () {
            statusbar.insert({
              type: 'label',
              name: 'wordcount',
              text: ['Liczba znaków: {0}, Liczba słów: {1}', getCount(), getWord() ],
              classes: 'wordcount',
